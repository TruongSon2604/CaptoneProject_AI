...
# Load ResNet50 bỏ lớp fully connected cuối (Cập nhật với weights)
model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)  # Dùng weights thay vì pretrained
model = torch.nn.Sequential(*list(model.children())[:-1])  # Bỏ FC cuối
model.eval()  # Chế độ đánh giá (không cập nhật trọng số)
